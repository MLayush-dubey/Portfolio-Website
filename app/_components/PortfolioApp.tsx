...
const PROJECTS = [
  { id: "01", name: "Hybrid RecSys", title: "BetBlitz live game recommender", company: "Quantum Horizon - Production", tag: "Recommender systems", blurb: "Solo-built an end-to-end LightFM hybrid recommender for live casino-game recommendations across roughly 100K users. Owned notebook-parity training, Prefect orchestration, FastAPI serving, zero-downtime model reloads, and production monitoring.", stack: ["LightFM", "WARP", "MongoDB", "Prefect", "FastAPI", "MLflow"], metrics: [["NDCG@10", "+34%"], ["CTR uplift", "+18%"], ["cold-start", "solved"]], href: "https://github.com/AyushDubeyaquagaming/Hybrid-RecSys", art: "matrix" },
  { id: "02", name: "Fraud-Detection", title: "Two-stage collusion fraud platform", company: "Quantum Horizon - In progress", tag: "Applied ML", blurb: "Production MLOps system for roulette partnership fraud: pair-level scoring feeds leakage-safe member alerting with shared real-time and batch scoring paths, automated promotion gates, and analyst review UI.", stack: ["scikit-learn", "FastAPI", "MongoDB", "MLflow", "Evidently", "Streamlit"], metrics: [["recall", "0.91"], ["precision", "0.88"], ["alerts/day", "-62%"]], href: "https://github.com/AyushDubeyaquagaming/Fraud-Detection", art: "anomaly" },
...
const CORPUS = [
  { id: "doc01", title: "Hybrid RecSys", chunk: "BetBlitz live casino-game recommender at Quantum Horizon using LightFM with WARP ranking loss. MongoDB data feeds a Prefect 2 AM pipeline, versioned model bundles, FastAPI + Uvicorn hot reload serving, MLflow tracking, Redis features, Neo4j embeddings, and Prometheus/Grafana monitoring. NDCG@10 improved 34%; CTR +18%; solved cold-start with item-feature embeddings." },
  { id: "doc02", title: "Fraud Detection", chunk: "Two-stage MLOps platform for roulette collusion fraud. Stage 1 scores suspicious player pairs with engineered features and class-balanced logistic regression; Stage 2 aggregates out-of-fold pair scores, CCS profit concentration, and analyst labels into member alerts. FastAPI real-time and batch scoring share DrawScorer; Streamlit supports analyst review. Recall 0.91, precision 0.88, alerts/day cut by 62%." },
  ...
  { id: "doc09", title: "Skills - Stack", chunk: "Python, PyTorch, TensorFlow, scikit-learn, LightFM, pandas, numpy, polars, PyArrow, BERT, HuggingFace, LangChain, CrewAI, LlamaIndex, RAG, ChromaDB, Pinecone, FastAPI, MLflow, Evidently, Prefect, Docker, Kubernetes, AWS." },
...
const EXPERIENCE = [
  ["02/2026 - Present", "AI/ML Engineer", "Quantum Horizon", "Dubai, UAE", ["Built and shipped a production hybrid recommender as the sole ML owner.", "Architecting a rules + ML fraud-detection system for real-time game exploits.", "Working across ML modelling, AWS infrastructure, and product analytics."], ["LightFM", "FastAPI", "AWS", "Production"]],
  ...
  ["06/2023 - 02/2024", "Data Scientist", "IndiqAI - Internship", "Mumbai", ["Built scalable Django web applications.", "Shipped 15+ RESTful endpoints with auth and validation.", "Executed 100+ automated API test cases."], ["Django", "DRF", "APIs", "Testing"]],
];

const CERTS = [
  ["MLA-C01", "AWS Certified Machine Learning Engineer", "Amazon Web Services", "Earned - May 2026", "earned", "https://www.credly.com/badges/f272108e-74ed-45ee-9473-8cb66b90aedc/public_url"],
  ["AWS-GENAI", "AWS Certified Generative AI Developer", "Amazon Web Services", "In progress - Target: July 2026", "pending"],
  ["NCA-GENL", "NVIDIA Certified Associate - Generative AI LLMs", "NVIDIA", "Planned - 2026", "planned"],
  ["NCA-AIAA", "NVIDIA Certified Associate - Agentic AI", "NVIDIA", "Planned - 2026", "planned"],
];
...
type ArchData = { nodes: ArchNode[]; edges: [string, string][]; callouts?: { label: string; value: string }[]; note?: string };
...
const ARCH: Record<string, ArchData> = {
  "01": { nodes: [
    { id:"mongo", x:35, y:44, w:95, h:44, label:"MongoDB", sub:"bets / games / players" },
    { id:"sim", x:35, y:128, w:95, h:44, label:"Sim data", sub:"approved signal" },
    { id:"prefect", x:178, y:86, w:100, h:50, label:"Prefect 2AM", sub:"flow / task" },
    { id:"feat", x:322, y:86, w:98, h:50, label:"Features", sub:"implicit_score" },
    { id:"train", x:462, y:44, w:104, h:48, label:"LightFM WARP", sub:"sample_weight" },
    { id:"bundle", x:462, y:128, w:104, h:48, label:"Versioned bundle", sub:"current.json" },
    { id:"api", x:610, y:86, w:105, h:50, label:"FastAPI + Uvicorn", sub:"hot reload" },
    { id:"go", x:735, y:86, w:52, h:50, label:"Go rec", sub:"top-N" },
    { id:"obs", x:610, y:164, w:105, h:36, label:"Prom + Grafana", sub:"MLflow / Redis / Neo4j" },
  ], edges: [["mongo","prefect"],["sim","prefect"],["prefect","feat"],["feat","train"],["train","bundle"],["bundle","api"],["api","go"],["feat","bundle"],["train","obs"],["api","obs"]],
  callouts: [
    { label:"training", value:"LightFM hybrid recommender with WARP ranking loss" },
    { label:"parity", value:"production pipeline reproduces notebook behavior" },
    { label:"bundle", value:"model, dataset, feature matrices, maps, metadata, popularity, version" },
    { label:"monitoring", value:"MLflow tracking with Redis, Neo4j, Prometheus, and Grafana support" },
    { label:"reload", value:"versioned directory + pointer swap + atomic rename + ModelService ref swap" },
  ], note:"Sanitized public view: production MongoDB host and database identifiers are intentionally omitted." },
  "02": { nodes: [
    { id:"mongo", x:34, y:52, w:98, h:44, label:"MongoDB", sub:"candidate pairs" },
    { id:"ccs", x:34, y:134, w:98, h:44, label:"CCS Parquet", sub:"profit signals" },
    { id:"pipe", x:174, y:92, w:108, h:50, label:"Training pipeline", sub:"Prefect + gates" },
    { id:"stage1", x:322, y:48, w:102, h:50, label:"Stage 1", sub:"pair scorer OOF" },
    { id:"stage2", x:322, y:136, w:102, h:50, label:"Stage 2", sub:"member alerts" },
    { id:"art", x:468, y:92, w:108, h:50, label:"Artifacts/current", sub:"manifest last" },
    { id:"serve", x:620, y:92, w:104, h:50, label:"FastAPI + batch", sub:"DrawScorer" },
    { id:"dash", x:742, y:92, w:48, h:50, label:"UI", sub:"review" },
    { id:"mlflow", x:468, y:164, w:108, h:36, label:"MLflow + Drift", sub:"Evidently" },
  ], edges: [["mongo","pipe"],["ccs","pipe"],["pipe","stage1"],["stage1","stage2"],["stage2","art"],["art","serve"],["serve","dash"],["pipe","mlflow"],["stage1","mlflow"],["stage2","mlflow"],["mlflow","art"]],
  callouts: [
    { label:"stage 1", value:"70 engineered pair features, impute + standardize + class-balanced logistic regression" },
    { label:"stage 2", value:"rolling member aggregation with pair scores, CCS concentration, analyst labels" },
    { label:"leakage", value:"Stage 2 trains only on out-of-fold Stage 1 scores" },
    { label:"promotion", value:"capture-rate@5% and lift@5% gate; bad models do not ship" },
    { label:"stack", value:"FastAPI, Uvicorn, Streamlit, MLflow, Evidently, Prefect, Docker Compose, pytest" },
  ], note:"Real-time and batch scoring share the same DrawScorer path." },
  "03": { nodes: [
...
function parseAction(text: string): { text: string; action: Action | null } {
  ...
  const action = m ? { type: m[1] as Action["type"], target: m[2], label: m[3] } : null;
  // Answers are plain text; strip the parsed action tag plus any residual/malformed tag-like
  // fragments (e.g. a model typo "<ction ...>") so they never leak into the chat bubble.
...
function Hero() {
  return <header id="top" className="hero"><LatentCanvas /><div className="wrap hero-grid"><div className="hero-copy"><HeroStatus /><h1><span>Ayush Dubey.</span><span className="dim glow">/* the AI/ML engineer */</span></h1><p>I build <b>LLM agents, recommender systems, and end-to-end ML pipelines</b> that survive contact with production. Currently architecting recsys and fraud-detection at <b>Quantum Horizon</b> in Dubai.</p><div className="hero-cta"><a className="btn primary" href="#projects">View work <Icon name="arrow" /></a><a className="btn" href="#playground">Try the RAG demo</a><a className="btn" href="/resume.pdf" download>resume.pdf <Icon name="download" /></a></div><div className="hero-socials"><a className="link" href="https://github.com/MLayush-dubey" target="_blank" rel="noopener noreferrer">github/MLayush-dubey</a><a className="link" href="https://www.linkedin.com/in/ayush-dubey-69860522a/" target="_blank" rel="noopener noreferrer">linkedin/ayush-dubey</a><a className="link" href="mailto:aadubey1106@gmail.com">aadubey1106@gmail.com</a></div></div><ChatAgent /></div></header>;
}
function HeroStatus() {
  return <div className="hero-status" aria-label="Current availability and location"><span className="hero-status-mark" /><span className="hero-status-item"><em>focus</em><b>Building production AI systems</b></span><span className="hero-status-item"><em>release</em><b>v2.5.1</b></span><span className="hero-status-item"><em>base</em><b>DXB - UTC+4</b></span></div>;
}
...
function ChatAgent() {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const parsed = parseAction(j.text || ""); const hasText = parsed.text.trim().length > 0; setMsgs((m) => [...m, { role: "bot", text: hasText ? parsed.text : "I couldn't generate a response just now - try rephrasing, or use a local command like whoami or ls projects.", action: hasText ? parsed.action : null }]);
...
function NowPlaying() {
  const items = [["currently reading","Attention Is All You Need (re-read)","paper"],["shipping","fraud-detect v0.4 - feature pipelines","wip"],["last commit","feat: add session-window aggregations","git"],["model in prod","hybrid-recsys-v2.3 - ~100K users","prod"],["now learning","AWS MLA-C01 - NCA-GENL","study"],["building toward","agentic eval harness with DSPy","lab"]];
...
function Projects() {
  ...
  return <section id="projects" className="sec"><div className="wrap"><SectionHead num="03 // projects" title="Featured work" meta="// 5 shipped - 1 in flight" /><div className="proj-grid">{PROJECTS.map(p=><div className="project-wrap" key={p.id}><article className="card pc" id={`project-${p.id}`}><ProjectArt kind={p.art}/><div className="pc-hd"><span>{p.id}</span><em>{p.tag}</em></div><h3><b>{p.name}</b><br/>{p.title}</h3><small>{p.company}</small><p>{p.blurb}</p><div className="metrics">{p.metrics.map(([k,v])=><span key={k}><b>{v}</b>{k}</span>)}</div><div>{p.stack.map(s=><Chip key={s}>{s}</Chip>)}</div><footer><a href={p.href} target="_blank" rel="noopener noreferrer"><Icon name="github" size={12}/> Source</a><button onClick={()=>setExpanded(expanded===p.id?null:p.id)}>{expanded===p.id?"hide arch":"view architecture"}</button></footer></article>{expanded===p.id&&<div className="arch"><header>// {p.id} - architecture <button onClick={()=>setExpanded(null)}>x</button></header><ArchDiagram id={p.id}/>{ARCH[p.id]?.callouts&&<div className="arch-callouts">{ARCH[p.id].callouts?.map(c=><span key={c.label}><b>{c.label}</b>{c.value}</span>)}</div>}{ARCH[p.id]?.note&&<p className="arch-note">{ARCH[p.id].note}</p>}<small>// data flows left to right - live trace animation</small></div>}</div>)}</div></div></section>;
}
...
function Certifications(){ const earned=CERTS.filter((c)=>c[4]==="earned").length; const active=CERTS.filter((c)=>c[4]==="pending").length; const planned=CERTS.filter((c)=>c[4]==="planned").length; const badge=(s:unknown)=>s==="earned"?"earned":s==="planned"?"planned":"in progress"; return <section id="certs" className="sec"><div className="wrap"><SectionHead num="05 // certifications" title="Credentials" meta={`// ${earned} earned - ${active} in flight - ${planned} planned`}/><div className="cert-grid">{CERTS.map(([code,title,org,when,status,url])=>{ const inner=<><span className={String(status)}>{badge(status)}</span><small>{code as string}</small><h3>{title as string}</h3><p>{org as string}</p><footer>{when as string}<b>{url?"verify ↗":status==="earned"?"verified":status==="planned"?"// queued":"// learning"}</b></footer></>; return url ? <a className="cert cert-link" key={String(code)} href={String(url)} target="_blank" rel="noopener noreferrer">{inner}</a> : <article className="cert" key={String(code)}>{inner}</article>; })}</div></div></section>; }
...
function CommandPalette(){ const [open,setOpen]=useState(false); const [q,setQ]=useState(""); const [sel,setSel]=useState(0); const input=useRef<HTMLInputElement|null>(null); const cmds=useMemo(()=>[["Go to - About","section 01",()=>location.hash="#about"],["Run - Live RAG demo","section 02",()=>location.hash="#playground"],["Go to - Projects","section 03",()=>location.hash="#projects"],["Go to - Experience","section 04",()=>location.hash="#experience"],["Go to - Certifications","section 05",()=>location.hash="#certs"],["Go to - Skills","section 06",()=>location.hash="#skills"],["Go to - Contact","section 07",()=>location.hash="#contact"],["Open - GitHub","MLayush-dubey",()=>openWindow("https://github.com/MLayush-dubey")],["Open - LinkedIn","ayush-dubey",()=>openWindow("https://www.linkedin.com/in/ayush-dubey-69860522a/")],["Toggle - Scanlines","crt",()=>document.body.classList.toggle("scanlines")],["Activate - Neural mode","secret",()=>document.body.classList.toggle("neural-mode")]],[]); const f=cmds.filter(c=>String(c[0]).toLowerCase().includes(q.toLowerCase())||String(c[1]).toLowerCase().includes(q.toLowerCase())); useEffect(()=>{ const keys=["ArrowUp","ArrowUp","ArrowDown","ArrowDown","ArrowLeft","ArrowRight","ArrowLeft","ArrowRight","b","a"]; let k=0; const h=(e:KeyboardEvent)=>{ const rawKey=typeof e.key==="string"?e.key:""; if(!rawKey) return; if((e.metaKey||e.ctrlKey)&&rawKey.toLowerCase()==="k"){e.preventDefault();setOpen(o=>!o)} else if(rawKey==="/"&&!/INPUT|TEXTAREA/.test((document.activeElement?.tagName)||"")){e.preventDefault();setOpen(true)} else if(rawKey==="Escape")setOpen(false); const key=rawKey.length===1?rawKey.toLowerCase():rawKey; if(key===keys[k]){ if(++k===keys.length){document.body.classList.toggle("neural-mode"); toast(); k=0;} } else k=key===keys[0]?1:0; }; addEventListener("keydown",h); return()=>removeEventListener("keydown",h);},[]); useEffect(()=>{if(open)setTimeout(()=>input.current?.focus(),20)},[open]); if(!open)return null; return <div className="palette-bg" onClick={()=>setOpen(false)}><div className="palette" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><div><span>&gt;</span><input ref={input} value={q} onChange={e=>{setQ(e.target.value);setSel(0)}} onKeyDown={e=>{const rawKey=typeof e.key==="string"?e.key:""; if(rawKey==="ArrowDown"){e.preventDefault();setSel(Math.min(sel+1,f.length-1))} if(rawKey==="ArrowUp"){e.preventDefault();setSel(Math.max(sel-1,0))} if(rawKey==="Enter"){(f[sel]?.[2] as Function)?.();setOpen(false)}}} placeholder="Type a command, file, or section..."/><kbd>ESC</kbd></div>{f.map((c,i)=><button className={i===sel?"sel":""} key={String(c[0])} onMouseEnter={()=>setSel(i)} onClick={()=>{(c[2] as Function)();setOpen(false)}}><span>{String(c[0])}</span><small>{String(c[1])}</small></button>)}</div></div>; }
function openWindow(url:string){ window.open(url,"_blank","noopener,noreferrer"); }
...
.hero{position:relative;min-height:calc(100vh - 80px);padding:80px 0 64px;display:flex;align-items:center;border-bottom:1px solid var(--line-soft);overflow:hidden}.latent-wrap{position:absolute;inset:-60px;opacity:.95;pointer-events:auto;mask-image:radial-gradient(ellipse 90% 80% at 60% 50%,#000 30%,transparent 95%)}.latent{display:block;width:100%;height:100%}.hero-grid{position:relative;display:grid;grid-template-columns:minmax(0,1.05fr) minmax(0,.95fr);gap:56px;align-items:center;z-index:2}.hero-copy{display:flex;flex-direction:column;gap:18px;max-width:640px}.row,.hero-cta,.hero-socials{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.hero-status{position:relative;width:max-content;max-width:100%;display:grid;grid-template-columns:4px repeat(3,max-content);align-items:stretch;gap:0;padding:5px 0 5px 10px;font:10px var(--f-mono);letter-spacing:.08em;text-transform:uppercase;color:var(--fg-dim)}.hero-status:before{content:"";position:absolute;left:0;right:0;top:50%;height:1px;background:linear-gradient(90deg,var(--accent),var(--line-soft),transparent);opacity:.45}.hero-status-mark{position:relative;z-index:1;width:4px;min-height:34px;background:linear-gradient(var(--accent),var(--accent-2));box-shadow:0 0 18px var(--accent-glow);clip-path:polygon(0 0,100% 8px,100% 100%,0 calc(100% - 8px))}.hero-status-item{position:relative;z-index:1;display:flex;flex-direction:column;justify-content:center;gap:2px;padding:4px 14px 4px 12px;background:oklch(0.105 0.012 250/.34);border-right:1px solid var(--line-soft);backdrop-filter:blur(6px)}.hero-status-item:nth-child(3){transform:translateY(4px)}.hero-status-item:nth-child(4){transform:translateY(-3px);border-right:0}.hero-status-item em{font-style:normal;font-size:8.5px;color:var(--fg-dim);letter-spacing:.16em}.hero-status-item b{color:var(--fg);font-weight:500;letter-spacing:.05em;white-space:nowrap}.hero h1 span{display:block}.hero-copy p{max-width:540px;color:var(--fg-mute);font-size:17px;line-height:1.6}.hero-copy b{color:var(--fg);font-weight:500}.hero-socials{gap:14px;margin-top:8px;padding-top:16px;border-top:1px solid var(--line-soft);font:12px var(--f-mono);color:var(--fg-mute)}
...
footer{border-top:1px solid var(--line-soft);padding:36px 0 80px;color:var(--fg-dim);font:11px var(--f-mono)}footer pre{color:var(--accent);opacity:.55;white-space:pre-wrap;overflow-wrap:anywhere;max-width:100%}
...
.nav-inner{padding:12px 16px;gap:10px}.brand{font-size:13px;gap:6px}.nav-links{gap:2px;margin-left:auto;flex-shrink:1;overflow-x:auto;scrollbar-width:none}.nav-links::-webkit-scrollbar{display:none}.nav-links a{padding:6px 6px;flex:0 0 auto}.nav-links button{display:none}
.hero{min-height:auto;padding:52px 0 72px;text-align:center}.hero-grid{gap:28px;justify-items:center}.hero-copy{align-items:center;max-width:560px}.hero .row,.hero-cta,.hero-socials{justify-content:center}.hero-status{width:100%;grid-template-columns:4px repeat(3,minmax(0,1fr));padding-left:8px;text-align:left}.hero-status-item{padding-inline:10px}.hero-status-item:nth-child(3),.hero-status-item:nth-child(4){transform:none}.hero-status-item b{white-space:normal;overflow-wrap:anywhere}.hero-copy p{font-size:15.5px;line-height:1.65;max-width:34rem}.hero h1{font-size:clamp(43px,13vw,60px)}.hero-socials{border-top:0;padding-top:4px;gap:10px 14px}
.chat-shell{width:0;height:0}.chat-shell>.chatframe{display:none}.ai-fab{display:grid;place-items:center;position:fixed;right:18px;bottom:calc(18px + env(safe-area-inset-bottom,0px));z-index:1300;width:58px;height:58px;border-radius:50%;border:1px solid var(--accent);background:oklch(0.105 0.012 250/.94);color:var(--accent);font:700 13px var(--f-mono);box-shadow:0 0 0 6px var(--accent-soft),0 18px 44px -12px #000;touch-action:manipulation;-webkit-tap-highlight-color:var(--accent-soft);transition:transform .12s}.ai-fab:active{transform:scale(.92)}.ai-fab:after{content:"";position:absolute;inset:8px;border-radius:50%;border:1px solid var(--accent-soft);animation:pulse 1.8s infinite}.chat-shell.open .chat-backdrop{display:block;position:fixed;inset:0;z-index:1190;border:0;background:oklch(0.06 0.01 250/.68);backdrop-filter:blur(8px)}.chat-shell.open>.chatframe{display:flex;position:fixed;left:14px;right:14px;bottom:88px;z-index:1201;height:min(72vh,560px);max-height:none;text-align:left;border-color:var(--accent);box-shadow:0 0 0 1px var(--accent-soft),0 28px 80px -18px #000}.chat-close{display:grid;place-items:center;width:24px;height:24px;border-radius:50%;border:1px solid var(--line);background:transparent;color:var(--fg-mute);font:12px var(--f-mono);margin-left:6px}.chat-hd b{margin-left:auto}.quick{overflow-x:auto;flex-wrap:nowrap;padding-bottom:2px}.quick button{flex:0 0 auto}.chat-input{gap:8px}.chat-input button{padding:6px 8px}.chat-input button svg{display:none}
.np .wrap{justify-content:center;text-align:center;gap:8px 14px}.np em{margin-left:0}.pg-body{padding:16px}.pg-q{align-items:stretch;flex-wrap:wrap}.pg-q input{flex-basis:100%;order:2}.pg-q button{order:3;width:100%;justify-content:center}.pipe{gap:10px}.stage{min-height:170px}.proj-grid{gap:14px}.pc{padding:20px;min-height:auto}.metrics{gap:14px}.arch{padding:16px;overflow-x:auto}.arch-svg{min-width:720px}.arch-callouts{grid-template-columns:1fr}.arch-callouts span{overflow-wrap:anywhere}.arch-note{overflow-wrap:anywhere}.skill-box{min-height:460px;padding:14px 10px 34px}.skill-legend{position:static;margin-bottom:8px;display:grid;grid-template-columns:1fr 1fr;font-size:9.5px}.skill-hint{left:12px;right:12px;text-align:center}.skill-meta{justify-content:center;text-align:center}.cta{text-align:center}.cta>div{grid-template-columns:1fr}.form{text-align:left}.edu{display:block}.cert h3{padding-right:92px}
}
`;
